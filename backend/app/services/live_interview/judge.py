...
from app.models import LiV2Session, LiV2Rubric, LiV2Evaluation
...
async def run_judge_pipeline(session_id: str):
    ...
    try:
        ...
        logger.error(f"[Judge] Pipeline failed for session {session_id}: {e}", exc_info=True)
...
async def _execute_pipeline(db, session_id: str):
    ...
    logger.info(f"[Judge] Phase A: segmenting {len(transcript)} turns across {len(dimensions)} dimensions")
    evidence_blocks = await _phase_a_segmentation(llm, transcript_text, dimensions)

    # --- Phase B: Anchor Matching (1=Substandard, 2=Proficient, 3=Excellent) ---
    logger.info(f"[Judge] Phase B: anchor matching")
    dimension_results = await _phase_b_anchor_match(llm, evidence_blocks, dimensions)

    # --- Phase C: Weighted Score ---
    logger.info(f"[Judge] Phase C: weighted score calculation")
    overall_score_pct, overall_score, coverage_ratio = _phase_c_score(dimension_results, dimensions)
...
async def _phase_a_segmentation(llm, transcript_text: str, dimensions: list[dict]) -> dict:
    """
...
{{
  ...
  ...
    ...
    try:
        ...
        logger.warning(f"[Judge] Phase A failed: {e} — using empty evidence")
...
async def _phase_b_anchor_match(llm, evidence_blocks: dict, dimensions: list[dict]) -> dict:
    """
...
{{
  ...
  "reasoning": "<1-2 sentence explanation>"
        ...
        try:
            ...
            logger.warning(f"[Judge] Phase B failed for dimension {dim_id}: {e}")
            results[dim_id] = {
                "score": 1,
                "anchor_matched": "substandard",
                "cited_quote": "",
...
def _phase_c_score(dimension_results: dict, dimensions: list[dict]) -> tuple[int, Decimal, Decimal]:
    """
...
async def _upsert_evaluation(
    db, session, dimension_results: dict,
    overall_score: Decimal, overall_score_pct: int,
    coverage_ratio: Decimal, verdict: str,
    meets_criteria: bool, confidence: str,
) -> LiV2Evaluation:
    ...
    evaluation.judged_at = datetime.now(timezone.utc)

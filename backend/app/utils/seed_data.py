...
# Initialize Faker
fake = Faker()
# Fallback to pbkdf2_sha256 to avoid bcrypt DLL issues on Windows
...
def generate_with_llama(prompt, context=""):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            prefixes = [
                "Here is a", "Sure, here is", "Certainly, here is", 
                "Here's a", "Sure thing", "Okay, here"
            ]
...
def seed_core_data():
    ...
    org_password_hash = pwd_context.hash("admin12345")
    ...
    cur.execute(
        ...
        VALUES (%s, %s, %s, %s, %s)
        """,
        (org_id, plan_ids["Enterprise"], "EraMatch Demo Corp", admin_email, org_password_hash)
    )
    
    # 3. USERS
    # Passwords hashed with admin12345 for simplicity or robust hash
    default_pw_hash = pwd_context.hash("admin12345")
    ...
    for p_name, p_status in project_configs:
        ...
        cur.execute(
            ...
            """,
            (pid, org_id, user_ids[admin_email], p_name, p_status, random.randint(10, 100))
        )
...
def generate_positions(org_id, user_ids, project_ids):
    ...
    for title in titles:
        ...
        level = "Senior Level" if "Senior" in title or "CTO" in title or "VP" in title or "Architect" in title else "Mid Level"
        if "Entry" in title or "Junior" in title: level = "Entry Level"
        ...
        try:
            ...
            if not isinstance(skills_list, list): raise ValueError
        except:
            # Fallback
            skills_list = ["General Tech", "Communication", "Problem Solving"]
            
        desc = generate_with_llama(f"Write a compelling 1-paragraph job description for a {title} position.")
        ...
        cur.execute(
            ...
            """,
            (pid, org_id, assigned_proj_id, title, desc, json.dumps(skills_list), 
             random.randint(80000, 120000), random.randint(130000, 180000), 
             hr_id, tech_id, level)
        )
    ...
    for pos_id in position_ids:
        ...
        for _ in range(count):
            ...
            cur.execute(
                ...
                (cid, org_id, name, email, fake.phone_number(), fake.city(), f"https://linkedin.com/in/{name.replace(' ', '')}", f"https://github.com/{name.replace(' ', '')}")
            )
            ...
            # Pass bio downstream
            candidates.append({"id": cid, "app_id": app_id, "status": status, "name": name, "pos_id": pos_id, "bio": bio})
...
def generate_detailed_assessments_and_interviews(org_id, candidates):
    ...
    for pid in unique_pos_ids:
        ...
        # Validate/Sanitize
        try:
            asm_structure_json = asm_structure_json.replace("```json", "").replace("```", "").strip()
            # Ensure it has 'sections'
            struct = json.loads(asm_structure_json)
            if "sections" not in struct: raise ValueError
        except:
            # Fallback Structure
            struct = {
                "sections": [
                    {"title": "Theory", "questions": [{"id": "q1", "text": f"Explain key concepts of {title}", "type": "mcq", "options": ["Concept A", "Concept B", "Concept C"]}]},
                    {"title": "Practice", "questions": [{"id": "q2", "text": "Solve a relevant problem", "type": "coding"}]}
                ]
            }
...
def generate_extra_artifacts(org_id, candidates, user_ids):
    ...
    for cand in candidates:
        ...
        cur.execute(
            ...
            """,
            (str(uuid4()), app_id, org_id, f"s3://cvs/{cand['name'].replace(' ', '_')}.pdf", ["Python", "FastAPI"], round(random.uniform(70, 99), 2))
        )
        ...
        if random.random() < 0.2 and status in ['rejected', 'interview']:
            ...
            cur.execute(
                ...
                """,
                (str(uuid4()), app_id, str(uuid4()), org_id, random.randint(100, 3000), flag_status)
            )
        ...
        if status in ['offered', 'hired']:
            ...
            cur.execute(
                ...
                """,
                (str(uuid4()), live_config_id, app_id, org_id, user_ids["tech@eramatch.com"], rating, decision)
            )
...
def generate_full_db_coverage(org_id, candidates, user_ids, position_ids, group_map):
    ...
    topics = ["Algorithms", "System Design", "Databases", "Networking", "Security", "DevOps", "Frontend", "Mobile", "AI/ML", "Soft Skills"]
        ...
        if cand['status'] in ['offered', 'hired']:
            ...
            if cand['status'] == 'hired':
                cur.execute(
                    ...
                    """,
                    (str(uuid4()), cand['app_id'], org_id, cand['app_id'], offer_id, salary)
                )
    ...
    for (sid,) in assessments_sessions:
        cur.execute(
            ...
            (str(uuid4()), sid, valid_qid, json.dumps("Dependency injection is a design pattern..."))
        )

...
from app.schemas.group import CloseStageRequest, CloseStageResponse
...
async def close_stage(
    ...
    """Close the current active stage for the group, locking further submissions."""
    svc = GroupService(session, current_user)
...
from app.schemas.group import BulkProgressionRequest, BulkProgressionResponse

@router.post(
    "/recruiter/groups/{group_id}/candidates/bulk-progress",
    response_model=BulkProgressionResponse,
)
async def bulk_progress_candidates(
    group_id: UUID,
    body: BulkProgressionRequest,
    session: DbSession,
    current_user: RecruiterUser,
):
    """Progress, reject, or hold a batch of candidates in one request."""
    svc = GroupService(session, current_user)
    return await svc.bulk_progress_candidates(
        group_id, body.application_ids, body.action, body.reason
    )


# ─── Send Offers ─────────────────────────────────────────────────────────────

from app.schemas.group import SendOffersRequest, SendOffersResponse

@router.post(
    "/recruiter/groups/{group_id}/offers/send",
    response_model=SendOffersResponse,
)
async def send_offers(
    group_id: UUID,
    body: SendOffersRequest,
    session: DbSession,
    current_user: RecruiterUser,
):
    """Create Offer records for selected candidates and mark their applications
    as offered."""
    svc = GroupService(session, current_user)
    return await svc.send_offers(
        group_id, body.application_ids, body.email_subject, body.email_body
    )

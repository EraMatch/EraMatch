"""
Assurance log script for Admin Login functionality.
Logs every step of the login flow with timing and detailed output.

Run: python tests/logs/log_admin_login.py
"""
import sys
import os
...
import httpx
from log_utils import AssuranceLogger, get_admin_token, make_client, BASE_URL


def run():
    log = AssuranceLogger("Admin Login")
    ...
    def test_valid_login():
        ...
        with httpx.Client(base_url=BASE_URL, timeout=30.0) as c:
            resp = c.post("/auth/admin/login", json={
    ...
    log.run("Valid admin login returns 200 + token", test_valid_login)
    ...
    def test_token_grants_access():
        ...
        with httpx.Client(base_url=BASE_URL, timeout=30.0) as c:
            resp = c.get("/admin/stats/global",
                         headers={"Authorization": f"Bearer {token}"})
        if resp.status_code != 200:
            return False, f"Protected endpoint returned {resp.status_code}: {resp.text[:200]}"
        return True, f"Protected endpoint accessible — status 200, keys={list(resp.json().keys())}"
    log.run("Token grants access to /admin/stats/global", test_token_grants_access)

    # ── 4. Wrong password blocked ─────────────────────────────────────
    ...
    # ── 6. Empty credentials blocked ─────────────────────────────────
    def test_empty_credentials():
        with httpx.Client(base_url=BASE_URL, timeout=30.0) as c:
            resp = c.post("/auth/admin/login", json={"email": "", "password": ""})
        if resp.status_code not in (400, 401, 422):
    ...
    # ── 9. Invalid token blocked ──────────────────────────────────────
    def test_invalid_token_blocked():
        with httpx.Client(base_url=BASE_URL, timeout=30.0) as c:
            resp = c.get("/admin/stats/global",
                         headers={"Authorization": "Bearer garbage.token.here"})
        if resp.status_code not in (401, 403):
            return False, f"Expected 401/403 with bad token, got {resp.status_code}"
        return True, f"Bad token correctly rejected with HTTP {resp.status_code}"
    log.run("Garbage token is rejected (401/403)", test_invalid_token_blocked)

"""
Shared utilities for EraMatch admin view assurance log scripts.
Provides: HTTP client with admin auth, timestamped logging, HTML/JSON report generation.
"""
import httpx
import json
import os
...
def get_admin_token() -> str:
    """Obtain admin JWT token. Exits with error message if login fails."""
    ...
    if resp.status_code != 200:
        print(f"  ❌  FATAL: Admin login failed ({resp.status_code}): {resp.text[:200]}")
        sys.exit(1)
    data = resp.json()
...
class AssuranceLogger:
    ...
    def __init__(self, module_name: str):
        ...
        print(f"\n{'='*60}")
        print(f"  EraMatch Admin Assurance Log")
...
<head>
    <meta charset="UTF-8">
    <title>EraMatch Admin Assurance — {summary['module']}</title>
    <style>
...
<body>
<div class="card">
    <h1>🛡️ EraMatch Admin Assurance Report</h1>
    <div class="meta">

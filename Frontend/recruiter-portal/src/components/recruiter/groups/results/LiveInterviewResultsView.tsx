...
interface LiveInterviewResultsViewProps {
    groupId: string;
    onOpenCandidate: (applicationId: string, candidateId: string) => void;
}

export function LiveInterviewResultsView({ groupId, onOpenCandidate }: LiveInterviewResultsViewProps) {
    const { data, isLoading, isError } = useStageMonitoring(groupId, 'live-interview');

    if (isLoading) {
        return <div className="space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="h-12 animate-pulse rounded-[10px] bg-gray-100" />)}</div>;
    }
    if (isError || !data) {
        return <div className="flex items-center justify-center rounded-[14px] border border-dashed border-red-200 bg-red-50 py-10"><p className="text-[13px] text-red-600">Failed to load live interview results.</p></div>;
    }

    const { total_candidates, completed, avg_score, candidates } = data as any;
    const withVerdict = (candidates as any[]).filter((c: any) => c.auto_verdict).length;
        ...
        <div className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
                {[
                    ...
                    { label: 'AI Evaluated', value: `${withVerdict} / ${completed}`, icon: Award, color: 'text-[#6366f1]' },
                    { label: 'Avg Score', value: avg_score > 0 ? `${(avg_score as number).toFixed(1)}%` : '—', icon: MessageSquare, color: 'text-blue-500' },
                ].map((kpi) => {
                    const Icon = kpi.icon;
                    return (
                    ...
                    <table className="w-full text-left text-[13px]">
                        <thead className="border-b border-gray-100 bg-gray-50">
                            <tr>
                                ...
                                <th className="px-4 py-3 font-semibold text-gray-600">AI Verdict</th>
                            ...
                            {(candidates as any[]).map((c: any, i: number) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3">
                                        {c.auto_verdict ? (
                                            <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${VERDICT_COLORS[c.auto_verdict] ?? 'bg-gray-100 text-gray-600'}`}>
                                                {c.auto_verdict}
                                            </span>
                                        ) : <span className="text-gray-400">—</span>}
                                    </td>
                                    <td className="px-4 py-3 tabular-nums text-gray-600">
                                        {c.overall_score_pct !== null && c.overall_score_pct !== undefined
                                            ? `${Math.round(c.overall_score_pct)}%`
                                            : '—'}
                                    </td>
                                    <td className="px-4 py-3">

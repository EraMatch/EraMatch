...
export function AdminPositionModal({ isOpen, onClose, onSuccess, projectId, position }: AdminPositionModalProps) {
    ...
    useEffect(() => {
        if (position) {
            ...
            setOfficeLocation(position.locationData?.office_location || '');
            setYearsOfExperience(position.yearsOfExperience || position.years_of_experience || 0);
            setEducationLevel(position.educationLevel || position.education_level || '');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload: any = {
                ...
                location_data: { office_location: officeLocation },
                ...
                <form onSubmit={handleSubmit} className="space-y-6 py-4 relative z-10">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input placeholder="e.g., Bachelor in CS" value={educationLevel} onChange={(e) => setEducationLevel(e.target.value)} className="bg-white/50 border-gray-200 rounded-xl" />
                        </div>

                        <div className="space-y-2">
                            <Label className="text-sm font-semibold text-gray-700">Salary Range (Minimum)</Label>
                            <div className="relative">
                                <Input type="number" value={salaryMin} onChange={(e) => setSalaryMin(parseInt(e.target.value))} className="bg-white/50 border-gray-200 pl-10 rounded-xl" />
                                <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                            </div>
                        </div>

                        <div className="space-y-2">
                            <Label className="text-sm font-semibold text-gray-700">Salary Range (Maximum)</Label>
                            <div className="relative">
                                <Input type="number" value={salaryMax} onChange={(e) => setSalaryMax(parseInt(e.target.value))} className="bg-white/50 border-gray-200 pl-10 rounded-xl" />
                                <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                            </div>
                        </div>
                    </div>

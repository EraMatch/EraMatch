...
- **Ollama desktop app** — Install from https://ollama.com (for cloud LLM models via local proxy)

### 1. Clone & Enter Directory
```bash
git clone <repo-url>
cd EraMatch/ai-service
```
...
> **Reproducibility**: `uv.lock` pins exact versions for all 460+ dependencies. `uv sync` creates an exact match of the development environment. For Docker or CI, use `requirements.txt` (generated by `uv export --format requirements-txt --no-dev`).
...
|----------|-------------|---------|
| `OLLAMA_BASE_URL` | Ollama proxy URL (local) | `http://localhost:11434` |
| `OLLAMA_API_KEY` | Leave empty for local proxy | (empty) |
| `INTERVIEWER_PRIMARY_MODEL` | LLM for interviewer | `gemma3:12b-cloud` |
...
# Start LiveKit Worker (separate terminal)
uv run python livekit_worker/agent_server.py dev
```
...
For Docker deployments, use the `requirements.txt` generated by `uv export`:
```bash
